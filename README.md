...
- 自定义：输入语言代码（如 ko、ar、hi、pt等，更多语言代码参考```whisper --help```）
...
- 建议在句子间隙或静音处标记切割点
- 避免在单词或句子中间切割
- 长视频建议每 5 分钟左右标记一个切割点
- 使用播放器的时间刻度快速定位

### 模型选择
- 模型可在cpu上运行，模型越大，识别越准确，但耗时更长
- `base` 模型适合快速识别，或单纯生成时间轴
- `turbo` 模型在速度和准确率之间平衡良好

### 工作区管理
- 建议每次识别都保存工作区，以便中断后恢复
- 工作区文件夹建议放在视频文件同目录下
- 识别完成后可使用"🗑️ Clean Up Temp Files"清理临时文件，或手动删除
- 保留工作区可以随时重新识别单个片段
...
- **窗口中显示的识别文本全是方块？** 由于GUI组件只支持拉丁文字，当识别中文、日文等，窗口会显示方块，但不影响最终结果。
